...
Quantization::Quantization(QWidget *parent) : HW(parent)
{
    m_level = 3.f;
}
...
void Quantization::initVertexBuffer()
{
...
void Quantization::initShaders()
{
    ...
    if(!m_program.addShaderFromSourceFile(QGLShader::Vertex, ":/vshaderQuant.glsl")) {
        QMessageBox::critical(0, "Error", "Vertex shader error", QMessageBox::Ok);
        QApplication::quit();
    }

    // compile fragment shader
    if(!m_program.addShaderFromSourceFile(QGLShader::Fragment, ":/fshaderQuant.glsl")) {
    ...
    m_uniforms[SCALE] = glGetUniformLocation(m_program.programId(),"u_scale");
    if((int) m_uniforms[SCALE] < 0){
...
void Quantization::setLevels(int level)
{
    m_level = level;
    glUniform1f(m_uniforms[SCALE], 1.f/m_level);
    updateGL();
...
void Quantization::initializeGL()
{
